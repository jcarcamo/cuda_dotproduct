...
__global__ void kernel_check_threads (int *force_d, int *distance_d)
{
    int x;
...
extern "C" void cuda_dotproduct (int *force, int *distance, int arraySize, int *result)
{
	...
	for (i=0; i<arraySize; i++){
        ...
        op_result = cudaMemcpy (result_d, result_array, sizeof(int) * arraySize, cudaMemcpyHostToDevice);
        if (op_result != cudaSuccess) {
                fprintf(stderr, "cudaMemcpy host->dev (result) failed.");
                exit(1);
        }
	
        int blocks = ceil(arraySize / ((float) BLOCK_SIZE));
